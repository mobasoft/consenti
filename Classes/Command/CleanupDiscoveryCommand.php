...
#[AsCommand(
    name: 'consenti:discovery:cleanup',
    description: 'Remove old or all records from consenti discovery table.'
)]
final class CleanupDiscoveryCommand extends Command
{
    protected function configure(): void
    {
        $this
            ->addOption(
                ...
                'Delete discoveries with last_seen older than N days',
                '90'
            )
            ->addOption(
                'all',
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_NONE,
                'Show number of affected rows without deleting'
            );
    }
    ...
    private function executeInternal(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$connection->createSchemaManager()->tablesExist(['tx_consenti_domain_model_discovery'])) {
            $output->writeln('Discovery table does not exist yet. Run database schema update first.');
            return Command::SUCCESS;
        }
        $queryBuilder = $connection->createQueryBuilder();
        $queryBuilder->count('uid')->from('tx_consenti_domain_model_discovery');
        ...
        if ($count === 0) {
            $output->writeln('No discovery records to delete.');
            return Command::SUCCESS;
        }

        if ($deleteAll) {
            $deleted = $connection->createQueryBuilder()
                ->delete('tx_consenti_domain_model_discovery')
                ->executeStatement();
        } else {
            $deleteQueryBuilder = $connection->createQueryBuilder();
            $deleted = $deleteQueryBuilder
                ->delete('tx_consenti_domain_model_discovery')
                ->where(
                    $deleteQueryBuilder->expr()->lt(
                        'last_seen',
                        $deleteQueryBuilder->createNamedParameter($threshold, ParameterType::INTEGER)
                    )
                )
                ->executeStatement();
        }
